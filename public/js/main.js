...
window.addEventListener("gamepadconnected", function(e) {
  window.setInterval(function () {
    ...
    if (gp.buttons[0].pressed) pressedKeys[keyCode['Space']] = true;
    else pressedKeys[keyCode['Space']] = false;
    if (gp.buttons[12].pressed || (gp.axes[1] < -0.5)) pressedKeys[keyCode['Up']] = true;
    else pressedKeys[keyCode['Up']] = false;
    if (gp.buttons[13].pressed || (gp.axes[1] > 0.5)) pressedKeys[keyCode['Down']] = true;
    else pressedKeys[keyCode['Down']] = false;
    if (gp.buttons[14].pressed || (gp.axes[0] < -0.5)) pressedKeys[keyCode['Left']] = true;
    else pressedKeys[keyCode['Left']] = false;
    if (gp.buttons[15].pressed || (gp.axes[0] > 0.5)) pressedKeys[keyCode['Right']] = true;    
    else pressedKeys[keyCode['Right']] = false;
  },100);
});
